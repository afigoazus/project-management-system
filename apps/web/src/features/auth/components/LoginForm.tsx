...
import { FolderKanban, Github, Mail, Lock, Loader2, ArrowRight } from "lucide-react";
import { useLogin, useGithubLogin } from "../hooks/auth.hook";
    ...
    <div className="min-h-screen flex flex-col justify-center items-center px-4 py-12 bg-[#0b0f19] relative overflow-hidden">
      ...
      <div className="w-full max-w-md space-y-6 relative z-10">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-slate-400">Sign in to your workspace account</p>
        </div>
        ...
        <div className="glass-card p-6 sm:p-8 rounded-2xl border border-slate-800 shadow-2xl space-y-6">
          ...
          <button
            ...
            type="button"
            disabled={isLoading}
            className="w-full flex items-center justify-center gap-2.5 py-2.5 px-4 rounded-xl bg-slate-900 hover:bg-slate-800 border border-slate-700 text-slate-200 hover:text-white font-medium text-xs transition-colors cursor-pointer disabled:opacity-50"
          >
