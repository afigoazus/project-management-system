...
import { useWorkspaceDetail } from "../hooks/workspace.hook";
...
export function WorkspaceDetailContent({ workspaceId }: WorkspaceDetailContentProps) {
  const { data: session, isPending: sessionPending } = useSession();
  const router = useRouter();

  const { data: workspace, isLoading: workspaceLoading, error: workspaceError, refetch } = useWorkspaceDetail(workspaceId);
  ...
  const isLoading = sessionPending || workspaceLoading;
